...
if torch.backends.cuda.is_built():
    device = torch.device("cuda")  # ✅ MPS 사용
    print("✅ cuda 사용 가능. cuda로 실행합니다.")
else:
    device = torch.device("cpu")  # ✅ CPU 사용
    print("❌ cuda 사용 불가능. CPU에서 실행합니다.")
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(parent_path, "classifier/bert_webtoon_classifier.pth")
    label_encoder_path = os.path.join(parent_path, "classifier/label_encoder.pkl")
    ...
    # ✅ 테스트 예측 실행
    synopsis = "평범한 고등학교로 유명한 청월고. 하지만 학교 깊숙한 곳에서는 강자들만 아는 불문율이 존재한다."
